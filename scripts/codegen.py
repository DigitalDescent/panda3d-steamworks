...
_GENERATED_BANNER = (
    "///\n"
    "// Copyright (c) " + str(datetime.datetime.now().year) + ", Digital Descent, LLC. All rights reserved.\n"
    "//\n"
    "// ======================================================================\n"
...
BYTES_BUFFER = "bytes_buffer"  # uint8*/size output pairs -> return PyObject*(bytes)
...
def classify_method(method, typedefs, enums):
    ...
    buffer_pairs = []       # (char_idx, size_idx) -> return std::string
    bytes_pairs = []        # (uint8_idx, size_idx) -> return PyObject*(bytes); size stays as wrapper param
    ...
    while i < len(params):
        ...
        if pt == "char *":
            ...
            # uint8* + int/uint32 size -> bytes output buffer
            if (i + 1 < len(params)
                    and params[i + 1]["paramtype"].strip() in ("int", "uint32", "int32")):
                bytes_pairs.append((i, i + 1))
                i += 2
                continue
            else:
                other_pointers.append(i)
    ...
    for ci, si in buffer_pairs:
        skip_indices.add(ci)
        skip_indices.add(si)
    for ci, _si in bytes_pairs:
        skip_indices.add(ci)   # only skip the uint8* pointer; size remains a real wrapper param
    for out_param in out_params:
    ...
    if bytes_pairs:
        return BYTES_BUFFER, bytes_pairs, []
    if out_params:
...
def _needs_string_include(methods_info):
    ...
    for _method, _kind, _bpairs, ret_info, _wrapper_params, _out_params in methods_info:
        if _kind == BYTES_BUFFER:
            return True  # uses std::string as internal buffer
...
def _has_bytes_buffer_methods(methods_info):
    """Check if any method is BYTES_BUFFER (also needs PyObject)."""
    return any(kind == BYTES_BUFFER for _, kind, _, _, _, _ in methods_info)


def _has_out_param_methods(methods_info):
    """Check if any method is OUT_PARAMS (returns PyObject* tuple)."""
    return any(kind == OUT_PARAMS for _, kind, _, _, _, _ in methods_info)


def _make_wrapper_params(method, buffer_pairs, out_params, typedefs, enums, kind=None):
    ...
    for ci, si in buffer_pairs:
        skip_indices.add(ci)
        if kind != BYTES_BUFFER:
            skip_indices.add(si)   # for bytes_buffer the size param stays visible
    for out_param in out_params:
        skip_indices.add(out_param["index"])
...
def _compute_call_args(method, bpairs, out_params, wrapper_params, kind=None):
    ...
    for ci, si in bpairs:
        skip_indices.add(ci)
        if kind != BYTES_BUFFER:
            skip_indices.add(si)   # for bytes_buffer the size is a real wrapper param
    out_param_by_index = {}
    for out_param in out_params:
        skip_indices.add(out_param["index"])
        ...
        if idx in skip_indices:
            if kind == BYTES_BUFFER:
                for ci, _si in bpairs:
                    if idx == ci:
                        call_parts.append("reinterpret_cast<uint8 *>(&buf[0])")
                        break
            else:
                for ci, _si in bpairs:
                    if idx == ci:
...
def _value_to_pyobject_expr(value_expr, cpp_type, special=None, is_enum=False):
    """Return a C++ expression that converts a scalar value to PyObject*."""
    if special == "steamid":
        return "PyLong_FromUnsignedLongLong((unsigned long long){}.ConvertToUint64())".format(value_expr)
    if is_enum:
        return "PyLong_FromLong((long)static_cast<int>({}))".format(value_expr)
    if cpp_type == "bool":
        return "PyBool_FromLong({} ? 1 : 0)".format(value_expr)
    if cpp_type in ("unsigned int", "unsigned short", "unsigned char"):
        return "PyLong_FromUnsignedLong((unsigned long){})".format(value_expr)
    if cpp_type == "unsigned long long":
        return "PyLong_FromUnsignedLongLong((unsigned long long){})".format(value_expr)
    if cpp_type == "long long":
        return "PyLong_FromLongLong((long long){})".format(value_expr)
    if cpp_type in ("int", "short", "char"):
        return "PyLong_FromLong((long){})".format(value_expr)
    if cpp_type in ("float", "double"):
        return "PyFloat_FromDouble((double){})".format(value_expr)
    return "PyLong_FromLong((long){})".format(value_expr)
...
def _prepare_source_method(method, kind, bpairs, ret_info, wrapper_params,
    ...
    if tmpl_kind == "simple":
        ...
        tuple_items = []
        if ret_info[0] != "void":
            tuple_items.append(_value_to_pyobject_expr("result", ret_info[0], ret_info[2], False))
        for out_param in out_params:
            tuple_items.append(_value_to_pyobject_expr(
                out_param["local_name"],
                out_param["return_type"],
                out_param["special"],
                out_param["is_enum"],
            ))
        ctx["tuple_items"] = tuple_items
    ...
    elif tmpl_kind == "bytes_buffer":
        ctx["ret_type_steam"] = method["returntype"].strip()
        ci, si = bpairs[0]
        ctx["bytes_size_param"] = steam_param_to_snake(method["params"][si]["paramname"])
        ctx["bytes_call_args"] = _compute_call_args(
            method, bpairs, out_params, wrapper_params, kind=BYTES_BUFFER)
...
def generate_header(iface_name, iface_data, iface_cfg, methods_info):
    ...
    return tmpl.render(
        ...
        has_async=_has_async_methods(methods_info),
            has_bytes_buffer=_has_bytes_buffer_methods(methods_info),
        has_out_params=_has_out_param_methods(methods_info),
    ...
    for iface_data in api_data.get("interfaces", []):
        ...
        for method in iface_data.get("methods", []):
            ...
            wrapper_params = _make_wrapper_params(
                method, bpairs, out_params, typedefs, enums, kind=kind)
            methods_info.append((
